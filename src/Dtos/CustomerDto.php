...
class CustomerDto implements IDtoFactory
{
    ...
    {
        if (!isset($customer['id'], $customer['email'], $customer['customer_code'])) {
        ...
        return new self(
            ...
            self::mapSubscriptions($customer['subscriptions'] ?? [])
        );
    }


    /**
     * Map subscription data to SubscriptionDto objects.
     *
     * @param array $subscriptions
     * @return \Illuminate\Support\Collection
     */
    private static function mapSubscriptions(array $subscriptions): \Illuminate\Support\Collection
    {
        return collect($subscriptions)->map(fn($data) => SubscriptionDto::create($data));
    }
}
